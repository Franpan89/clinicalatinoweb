...
import { toggleNewsActive } from '@/app/admin/actions'

export default function ToggleNewsActive({ id, active }: { id: string; active: boolean }) {
  const [isPending, startTransition] = useTransition()
  const [isActive, setIsActive] = useState(active)

  const handleToggle = () => {
    startTransition(async () => {
      const result = await toggleNewsActive(id, isActive)
