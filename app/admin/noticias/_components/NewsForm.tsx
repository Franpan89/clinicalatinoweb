...
export default function NewsForm({ mode, news }: { mode: Mode; news?: News }) {
  const [error, setError] = useState<string | null>(null)
  const [isPending, startTransition] = useTransition()
  const [coverPreview, setCoverPreview] = useState<string | null>(news?.cover_image_url ?? null)
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    startTransition(async () => {
      const result =
        mode === 'create' ? await createNews(formData) : await updateNews(news!.id, formData)
    ...
    <div className="container mx-auto py-10 max-w-4xl">
      ...
      <div className="mb-8">
        <h1 className="font-lato text-brand-dark text-3xl font-bold mb-2">
          {mode === 'create' ? 'Agregar noticia' : `Editar: ${news?.title}`}
        </h1>
        <p className="font-lato text-brand-gray">
          Completa el contenido de la noticia o novedad.
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-8">
        <Section title="Imagen de portada" subtitle="Opcional · Máx. 5MB · JPG, PNG o WebP">
          <div className="flex items-start gap-6">
            <div className="w-40 h-28 bg-brand-surface border-2 border-dashed border-brand-dark/15 flex items-center justify-center overflow-hidden flex-shrink-0">
              {coverPreview ? (
            ...
            <div className="flex-1">
              <label className="inline-flex items-center gap-2 cursor-pointer bg-white border border-brand-dark/20 hover:border-brand-teal text-brand-dark font-lato text-sm font-bold px-4 py-2.5 transition-colors">
                <Upload size={14} />
                {coverPreview ? 'Reemplazar imagen' : 'Subir imagen'}
                <input
                  ref={fileInputRef}
                  type="file"
                  name="cover_image"
              ...
              {removeCover && <input type="hidden" name="remove_cover_image" value="on" />}
            </div>
          </div>
        </Section>
        ...
        <Section title="Contenido">
          ...
          <div className="mt-5">
            <Field label="Extracto *" hint="1-2 líneas que aparecen en la tarjeta de listado">
              <textarea
                name="excerpt"
                required
                rows={2}
          ...
          <div className="mt-5">
            <Field label="Contenido *" hint="Los saltos de línea se preservan automáticamente">
              <textarea
                name="content"
                required
                rows={12}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            <Field label="Fecha de publicación">
              <input
                type="date"
                name="published_at"
                defaultValue={news?.published_at ?? new Date().toISOString().split('T')[0]}
                className={inputClass}
              />
            </Field>
            ...
            >
              <input
                type="number"
                name="display_order"
                defaultValue={news?.display_order ?? 999}
                className={inputClass}
              />
            </Field>
        ...
        <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-brand-surface">
          ...
          >
            ...
            {isPending ? 'Guardando...' : mode === 'create' ? 'Crear noticia' : 'Guardar cambios'}
          </button>
          <Link
            href="/admin/noticias"
