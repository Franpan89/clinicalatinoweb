...
import { deleteInsuranceLogo } from '@/app/admin/actions'

export default function DeleteInsuranceLogoButton({ id, name }: { id: string; name: string }) {
  ...
  const handleDelete = () => {
    ...
    startTransition(async () => {
      const result = await deleteInsuranceLogo(id)
      if (result?.error) {
      ...
      <div className="bg-white max-w-md w-full p-6 border border-red-200 text-left">
        <div className="flex items-start gap-4 mb-5">
          ...
          <div className="flex-1">
            ...
            <p className="font-lato text-brand-gray text-sm leading-relaxed">
              Esta acción es permanente. El logo dejará de aparecer en el slider del home.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            ...
            disabled={isPending}
            className="font-lato text-sm font-bold px-5 py-2.5 bg-red-600 hover:bg-red-700 text-white transition-colors disabled:opacity-50"
          >
